...
import com.eazybytes.loans.entity.ErrorResponseDto;
...
public class LoansController {
    ...
    public ResponseEntity<LoansDto> fetchLoanDetails(@RequestHeader("eazybank-correlation-id") String correlationId,
            ...
            @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile Number must be 10 digits")
            String mobileNumber){
        log.debug("Inside LoansController.fetchLoanDetails(), correlationId: {}", correlationId);
    ...
    @PutMapping("/")
    public ResponseEntity<ResponseDto> updateLoanDetails(@Valid @RequestBody LoansDto theLoansDto){

        boolean isUpdated = loansService.updateLoanDetails(theLoansDto);
        if(isUpdated)
            return ResponseEntity.status(HttpStatus.OK)
